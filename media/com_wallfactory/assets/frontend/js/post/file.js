jQuery(document).ready(function ($) {
    var $template = $('#media-file-template');
    var config = $('div.submit-post-buttons').data('config');
    var $file = $('input#media-file-template-file');
    var fileConfig = $('script[data-media="file"]').data('config');

    $(document).on('media_button_file', 'div.submit-post-buttons', function () {
        $file.click();
    });
    ...
    $file.change(function () {
        ...
        for (var i = 0, count = this.files.length; i < count; i++) {
            ...
            if (-1 === $.inArray(extension, fileConfig.valid_extensions)) {
                continue;
            }
            ...
            $('div.submit-post-buttons').trigger('insert', [template, function () {
                ...
                this.addClass('file-pending-upload').data('file', file);
    ...
    function startUpload() {
        var $pending = $('div.file-pending-upload:first', 'div.submit-post-media');
        ...
        var fd = new FormData();
        fd.append('file', $pending.data('file'));
        fd.append('type', 'file');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    $preview.text(response.output.name);
            ...
            complete: function () {
                $pending.removeClass('file-pending-upload').data('file', undefined);
                startUpload();
            }
        });
    }
